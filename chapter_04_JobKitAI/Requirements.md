Create a local-first Job Tracker as a single-page React application scaffolded with Vite. All data must persist in the browser using IndexedDB (use the `idb` library for a cleaner async API). No backend or authentication is needed.
 once basic Job tracker application is done keep code scalable such way that later able to add a seprate Page Evaluate a job based on shared Resume and Job descrption. Later provide details aboot Evalua a job board.
...
- Evaluate Job board: fields
   Company Name field, Designation field, Job description (user able to enter description max keep standard like linkedin or other job platforms JDS )
 -Submit button (name that button)
 After resume is sumbitted display strength and gaps for that JD properly